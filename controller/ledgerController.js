...
exports.withdrawFundsForVehicle = async ({
  driverId, supervisorId, vehicleId, amount, tripId, builtyId = null, type = "WITHDRAW", actionBy, expenseModel = "Vehicleexpense", expenseId
}) => {
...
exports.withdrawFundsForDriver = async ({
  driverId, supervisorId, vehicleId, amount, tripId, builtyId = null, actionBy, expenseModel = "DriverExpense", expenseId
}) => {
...
// exports.updateLedgerForAmountChange = async (expenseId, newAmount, actionBy, type = 'WITHDRAW') => {
//   console.log(`[Ledger Service] Updating ledger for Expense ID: ${expenseId} with new amount: ${newAmount}`);
//   const ledgerEntry = await WalletLedger.findOne({ expenseId });
//   if (!ledgerEntry) throw new Error("Ledger entry not found");
//   console.log(`[Ledger Service] Found ledger entry: ${ledgerEntry._id} with current amount: ${ledgerEntry.amount}`);

//   const updatedAmount = -Math.abs(Number(newAmount));
//   const balanceDifference = updatedAmount - ledgerEntry.amount;

//   ledgerEntry.amount = updatedAmount;
//   ledgerEntry.actionBy = actionBy;

//   return await ledgerEntry.save();
// };

exports.updateLedgerForAmountChange = async (expenseId, newAmount, actionBy) => {
  const ledgerEntry = await WalletLedger.findOne({ expenseId });
