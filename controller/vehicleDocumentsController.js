...
exports.addVehicleDocument = async (req, res) => {
  try {
    const { deviceId, documents } = req.body;

    if (!deviceId) {
      return res.status(400).json({ message: "deviceId is required" });
    }

    const vehicle = await VehicleMaster.findById(deviceId).select(
      "vehicleNumber"
    );
    ...
    const existingDocument = await VehicleDocument.findOne({ deviceId });
...
exports.updateVehicleDocument = async (req, res) => {
    try {
        ...
        const existingDocument = await VehicleDocument.findOne({ deviceId: req.params.id });
...
exports.getVehicleDocument = async (req, res) => {
    try {
        const { deviceId, field } = req.query
        const vehicleDocument = await VehicleDocument.findOne({ deviceId }).select(`documents.${field}`);
...
exports.deleteVehicleDocumentImage = async (req, res) => {
    try {
        ...
        if (field === 'Insurance') {
            ...
            await VehicleDocument.findOneAndUpdate({ deviceId }, {
...
exports.getVehicleExpiryDocuments = async (req, res) => {
    try {
        ...
        if (vehicleIds.length > 0) vehicleQuery.deviceId = { $in: vehicleIds };
        ...
        const result = expiringDocuments.map(doc => {
            const vehicle = vehicles.find(v => v._id.toString() === doc.deviceId.toString());
